//! How many crates depend on each crate.

use crate::index::CrateIndex;
use crate::store::Store;
use core::ops::Index;

/// The number of crates depending on every crate, addressed by [`CrateIndex`].
#[derive(Debug, Clone, Default, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "serde", serde(transparent))]
pub struct Dependents(Store<u32>);
...
impl Dependents {
    ...
    /// Appends `dependents` and returns the index it was given.
    ///
    /// # Panics
    ///
    /// Panics if the store already holds [`u32::MAX`] counts.
    pub fn push(&mut self, dependents: u32) -> CrateIndex {
        self.0.push(dependents)
    }

    /// Returns the count at `index`, or `None` if the index is unoccupied.
    pub fn get(&self, index: CrateIndex) -> Option<u32> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (CrateIndex, u32)> + '_ {
        self.0
            .iter()
...
impl Index<CrateIndex> for Dependents {
    type Output = u32;

    /// # Panics
    ///
    /// Panics if the index is unoccupied.
    fn index(&self, index: CrateIndex) -> &u32 {
        self.0.get(index).expect("no count at this index")
    }
}
...
mod tests {
    ...
    fn counts_of(values: &[u32]) -> Dependents {
        let mut counts = Dependents::new();
        for value in values {
            counts.push(*value);
        }
        counts
